...
6. Find good epsilon
7. DBSCAN
Then: Look into clusters and see if they make sense. Then try to implement a clustering of the texts based on the vectors of the keywords

Add-Ons:
- Filter all keywords that are only being used once (and count them, I want to know how many there are)
    Dropping rare words: https://stackoverflow.com/questions/61460683/word2vec-using-document-body-or-keywords-as-training-corpus
    ...
    for index, row in df.iterrows():
        ...
        for i in df.index[index:]:
            if df['identifier'][i] != identifiers[-1]:
                break
            entry = re.sub("\s", "-", df['keyword'][i]) # Changes whitespace characters to underlines, so as to avoid further issues with the tokenization
...
# Save to csv
def save_csv(filename, lst, keyword):
...
def read_to_list(filename, keyword):
    lst = []
...
def get_keyword_counts(sequences):
    keyword_counts = {}
...
#print('Most used keyword: ', max(keyword_counts, key=keyword_counts.get), ', used ', max(keyword_counts.values()), ' times')
...
#print(embeddings[0:10])                 # Holds the vectorized vocabulary in a nested list. Each vector is a list of 10 floats
